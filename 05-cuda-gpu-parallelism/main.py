...
for epoch in range(N_EPOCH):
    for i, (images, labels) in enumerate(trainloader):
        ...
        images = images.view(-1, D_INPUT).to(device)
        noises = torch.randn(images.size(0), D_NOISE).to(device)
        labels = onehot(labels, N_CLASSES).to(device)
        ...
        fakes = torch.cat((G(noises), labels), 1).to(device)
        ...
        D_image_loss = criterion(D_images, torch.ones(images.size(0)).to(device))
        D_fake_loss = criterion(D_fakes, torch.zeros(images.size(0)).to(device))
        ...
        G_loss = criterion(D(G_fakes), torch.ones(images.size(0)).to(device))
        G_loss.backward()
        G_optim.step()
    ...
    test_noises = torch.cat((test_noises, torch.eye(10)), 1).to(device)
